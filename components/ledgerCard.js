...
const LedgerCard = (props) =>{
    return(
        <div className="ledger_card">
            ...
            fontWeight:'300'}}><BsBoxArrowInDown /><div style={{marginLeft:"0.1rem"}}>{props.status=='PROD_RETURN'?<div>Production Return</div>:<div>Stock In</div>}</div></div>
            :
            <div className='stock_out_style' style={{margin:'0 5px',fontWeight:'300'}}><BsBoxArrowUp /><div style={{marginLeft:"0.1rem"}}>{props.status =='LINE_LOSS'?<div>Loss On Line</div>:<div>Stock Out</div>}</div></div>}</div>
